...
#define OT_MIPI_SET_HS_MODE      _IOW('m', 0x0b, int)
#define OT_MIPI_ENABLE_SENSOR_CLOCK _IOW('m', 0x10, int)
#define OT_MIPI_RESET_SENSOR     _IOW('m', 0x05, int)
#define OT_MIPI_UNRESET_SENSOR   _IOW('m', 0x06, int)
...
void enable_mclk(void) {
    ...
    ioctl(fd, OT_MIPI_RESET_SENSOR, &rst);
    usleep(10000);
    ioctl(fd, OT_MIPI_UNRESET_SENSOR, &rst);
    usleep(100000);
    close(fd);
    fprintf(stderr, "MCLK enabled, sensor reset released\n");
}
