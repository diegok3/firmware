        ...
        while (g_running) {
            ...
            if (select(client_fd + 1, &rfds, NULL, NULL, &tv) > 0) {
                ...
                if (n < 0 && errno != EAGAIN && errno != EWOULDBLOCK) {
                    break; /* real error → client disconnected */
                } else if (n > 0) {
            ...
            if (ret != TD_SUCCESS) {
                consecutive_fail++;
                fprintf(stderr, "  [DBG] get_frame fail %d, ret=0x%x\n", consecutive_fail, ret);
                fflush(stderr);
            ...
            fprintf(stderr, "  [DBG] frame: %ux%u stride=%u phys=0x%llx size=%llu\n",
                    vf->width, height, stride, (unsigned long long)phys, (unsigned long long)map_size);
            fflush(stderr);
            ...
            if (!mem_mmaped || mem_map_phys != page_phys || mem_map_len != map_len) {
                ...
                if (mem_map == MAP_FAILED) {
                    fprintf(stderr, "  [DBG] mmap FAILED phys=0x%llx len=%llu errno=%d\n",
                            (unsigned long long)page_phys, (unsigned long long)map_len, errno);
                    fflush(stderr);
                    ot_mpi_vi_release_chn_frame(VI_PIPE_ID, VI_CHN_ID, &frame_info);
                    break;
                }
